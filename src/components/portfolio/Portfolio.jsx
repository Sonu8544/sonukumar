      ...
      <div className="container portfolio__container">
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {imageOverlayData.map((item) => (
            <div
              key={item.id}
              className="image-container border-2 relative"
              onMouseEnter={() => handleMouseEnter(item.id)}
              onMouseLeave={handleMouseLeave}
            >
              <img
                src={hovered === item.id ? item.imgSrc2 : item.imgSrc}
                alt={`Avatar ${item.id}`}
                className="image"
                style={{ width: "100%" }}
              />
              <div className="absolute bottom-0 left-0 right-0 bg-gray-500 bg-opacity-75 text-white text-center p-2">
